...
#kubectl -n mongoshard scale sts mongodbshardstateful --replicas=4
#sleep 5
primaryshard=''
...
shardarray=(`kubectl -n mongoshard get --no-headers=true pods -l role=mongoshard -o custom-columns=:metadata.name`)
 for i in ${shardarray[@]};do
  primary=`kubectl -n mongoshard exec $i -c ${shard_container} -- mongo --quiet --eval 'rs.isMaster().ismaster'`
  if [ $primary = 'true' ];
    then
       primaryshard=$i
       break
...
for i in 3; do
 kubectl -n mongoshard exec ${primaryshard} -c ${shard_container}  -- bash -c "echo \
