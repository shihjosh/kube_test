...
getfqdn_pods() {
 ...
 num=0
 configarray=(`kubectl -n mongoshard get --no-headers=true pods -l role=mongoconfig -o custom-columns=:metadata.name`)
 for i in ${configarray[@]};do
  echo $i
  config1pod[num++]=`kubectl -n mongoshard exec $i -- hostname -f`
 done
}
...
replicasetInit_ShardandConfig() {
   ...
   kubectl -n mongoshard exec ${config_shortName}-0 -c ${config_container} -- bash -c "echo 'rs.initiate({_id:\"${config_replicasetName}\",\
   ...
   #kubectl -n mongoshard exec ${config_shortName}-0 -c ${config_container} -- mongo --eval 'rs.status();'
   ...
   kubectl -n mongoshard exec ${shard_shortName}-0 -c ${shard_container} -- bash -c "echo 'rs.initiate({_id :\"${shard_replicasetName}\",\
   members: [{ _id:0, host:\" ${shard1pod[0]}:${mongoport}\" },{ _id:1, host: \"${shard1pod[1]}:${mongoport}\" },{ _id:2, host: \"${shard1pod[2]}:${mongoport}\" }]})'|mongo "
   sleep 5
   
   #get shard status
   #kubectl -n mongoshard exec ${shard_shortName}-0 -c ${shard_container}  -- mongo --eval 'rs.status();'
}
...
replicasetInit_statuscheck() {
   ...
   for i in 0 1 2; do
     shardstatus=`kubectl -n mongoshard exec ${config_shortName}-${i} -c ${config_container} -- mongo --quiet --eval 'rs.status().myState;'` 
     echo "shard_status:"$shardstatus 
     kubectl -n mongoshard exec ${config_shortName}-${i} -c ${config_container} -- mongo --quiet --eval \
   ...
   for i in 0 1 2; do
     shardstatus=`kubectl -n mongoshard exec ${shard_shortName}-${i} -c ${shard_container} -- mongo --quiet --eval 'rs.status().myState;'` 
     echo "shard_status:"$shardstatus
     kubectl -n mongoshard exec ${shard_shortName}-${i} -c ${shard_container} -- mongo --quiet --eval \
     'while ((rs.status().hasOwnProperty("myState")) && ((rs.status().myState == 0) )) { print("..."); sleep(2000); };'
   done
...
<< "Comment"
addshard_router() {
  kubectl -n mongoshard create -f "/home/yjayapra/.kube/mongodbRouter.yaml"
  echo "waiting to get status from mongos routers..."
  sleep 5
  #TODO wait until all the pods are tsrated and in ready state  
  until kubectl -n mongoshard --v=0 exec ${router_pod1} -c ${router_container1} -- mongo --quiet --eval 'db.getMongo()'; do
    sleep 5
    echo -n "  "
  done
