...
Comment
  ...
  kubectl -n mongoshard exec ${shard_shortName}-0 -c ${shard_container} -- bash -c "echo 'rs.initiate({_id :\"${shardreplicaset}\",\
 ...
 #Add Shards on routers
  ...
  kubectl -n mongoshard exec ${router_pod1}  -c ${router_container1} -- bash -c "echo 'sh.addShard(\"${shardreplicaset}/${shard2pod}:${mongoport}\")' |mongo "
